...
public class BmwTorontoProvider : ICarSearchProvider
{
    private readonly BmwTorontoSnapshotParser _parser;
    private readonly IServiceProvider _serviceProvider;
    private readonly ProviderOptions _options;
    private readonly PlaywrightCliOptions _cliOptions;
    private readonly ILogger<BmwTorontoProvider> _logger;

    public string Name => "BmwToronto";
    public string DisplayName => "BMW Toronto";
    public bool IsEnabled => _options.Enabled;

    public BmwTorontoProvider(
        BmwTorontoSnapshotParser parser,
        IServiceProvider serviceProvider,
        IOptionsSnapshot<ProviderOptions> options,
        IOptions<PlaywrightCliOptions> cliOptions,
        ILogger<BmwTorontoProvider> logger)
    {
        _parser = parser;
        _serviceProvider = serviceProvider;
        _options = options.Get("BmwToronto");
    ...
    public async Task<ProviderSearchResult> SearchAsync(SearchParameters parameters, CancellationToken ct = default)
    {
        ...
        try
        {
            var inventoryUrl = _options.BaseUrl.TrimEnd('/') + "/used-vehicles";
            ...
            var makeRef = _parser.FindButtonRef(yaml, parameters.Make);
            if (makeRef != null) { await cli.ClickAsync(makeRef); await cli.WaitAsync(2000); }
            yaml = await cli.SnapshotWithRetryAsync();
            var modelRef = _parser.FindButtonRef(yaml, parameters.Model);
            if (modelRef != null) { await cli.ClickAsync(modelRef); await cli.WaitAsync(2000); }
            if (!string.IsNullOrEmpty(parameters.Color))
            {
                yaml = await cli.SnapshotWithRetryAsync();
                var colorRef = _parser.FindColorRef(yaml, parameters.Color);
                if (colorRef != null) { await cli.ClickAsync(colorRef); await cli.WaitAsync(2000); }
            }
