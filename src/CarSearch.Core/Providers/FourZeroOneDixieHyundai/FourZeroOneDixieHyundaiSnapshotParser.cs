...
public class FourZeroOneDixieHyundaiSnapshotParser
{
    ...
    {
        var pattern = @"button\s+""(?:Used|Pre-Owned)\s+vehicles?\s+(\d+)""";
        var match = Regex.Match(yaml, pattern);
    ...
    public List<VehicleListing> ParseListings(string yaml)
    {
        ...
        while (i < lines.Length)
        {
            ...
            var listing = new VehicleListing
            {
                ...
                Dealer = "401 Dixie Hyundai",
                Source = "FourZeroOneDixieHyundai"
            };
            var blockEnd = Math.Min(i + 30, lines.Length);
            var block = string.Join('\n', lines[i..blockEnd]);
            var urlMatch = Regex.Match(block, urlPattern);
            if (urlMatch.Success)
                listing.Url = "https://www.401dixiehyundai.com" + urlMatch.Groups[1].Value;
            ...
            if (listing.Year >= 2000) listings.Add(listing);
            i++;
        }
        return listings;
    }
